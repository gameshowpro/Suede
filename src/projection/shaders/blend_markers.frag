// The seam-boundary marker variant of blend.frag, used only while the
// `highlightOverlaps` lineup aid is on. Rebuild with
// `naga --input-kind glsl --shader-stage frag blend_markers.frag blend_markers.frag.spv`.
//
// This file is blend.frag verbatim, below this header, plus the one block
// between the "begin markers" and "end markers" lines in `main`. The
// `marker_shader_is_the_plain_shader_plus_the_marker_block` test in gpu.rs
// holds it to that, so edit blend.frag first and copy the change here.
//
// It is a separate file, not a specialization constant, because naga's
// SPIR-V backend bakes a GLSL `constant_id` constant to its default instead
// of emitting a real OpSpecConstant. A second file is also what keeps the
// plain pipeline exactly the shader it was before markers existed.
//
// In this variant bits 28..29 of every transfer entry, fixed or dynamic,
// carry the marker tag: 0 none, 1 orange, 2 blue. The
// comments below that call those bits zero or reserved describe the plain
// shader's input. See `blend::MARKER_NONE` in blend.rs for why they are free.
// ---- end of variant header ----
...
void main() {
    ...
    // ---- begin markers ----
    // The marker is a flat, full-strength color that replaces the whole
    // transfer below instead of passing through it. The blue line marks
    // where this output's own blend weight reaches zero, so a marker
    // multiplied by its own transfer would vanish exactly where it is drawn.
    // Returning here also keeps a marked pixel off the canvas, because its
    // transfer entry may be zero (see the `ab == 0u` return below).
    // `Blend::marked_rows` and `SyncPaint::marked_rows` in slicer.rs do the
    // same, so both renderers still agree byte for byte.
    // The green here is the palette's at gamma 2.2. `marker_spirv` in gpu.rs
    // replaces it in the compiled module with the configured gamma's.
    uint marker = (ab >> 28u) & 0x3u;
    if (marker == 1u) {
        outColor = vec4(vec3(255.0, 186.0, 0.0) / 255.0, 1.0);
        return;
    } else if (marker == 2u) {
        outColor = vec4(vec3(0.0, 186.0, 255.0) / 255.0, 1.0);
        return;
    }
    // ---- end markers ----
