...
impl CanvasRect {
    ...
    pub fn pixel_rect(self, canvas: &CanvasConfig) -> [f64; 4] {
        let (width, height) = canvas
            .dimensions()
            .expect("pixel_rect requires a valid canvas configuration");
...
impl OutputGeometry {
    ...
    pub fn validate_for_output(
        ...
        crate::warp_math::Warp::new(corners, self.center, width, height)
            .and_then(|warp| warp.with_source_rect(self.source.pixel_rect(canvas)))
            .map(|_| ())
            .map_err(|error| format!("geometry corners are invalid: {error}"))
    }
}
...
mod tests {
    ...
    fn dimensions_round_height_and_preserve_density() {
        ...
        assert_eq!(
            rect(0.0, 0.0, 0.5, 0.5).pixel_rect(&canvas),
            [0.0, 0.0, 950.0, 950.0]
        );
    }
