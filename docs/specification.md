...
| `PUT` | `/config/projection/arrangement` | Solve a rows × columns grid (per-axis overlap, or a content scale) and write each enabled output's `geometry.source`; `committed: false` (default) previews, `true` persists. Returns the full document, like any other config write, with the resolved record as `projection.arrangement`. See [Grid arrangement](configuration.md#grid-arrangement). |
| `GET` | `/config/projection/arrangement` | The resolved record of the last applied arrangement, whether re-solving it still reproduces the current sources (`inEffect`), and (when computable) its per-axis overlap `limits`, always as if `allowUnusedCanvas` were `false`. |
