...
| `GET` | `/projection/arrangement` | Read-only dry run of the grid-arrangement solver for the current working copy: query `rows`, `columns`, and either `overlapX`+`overlapY` or `contentScale`. Overlaps are honored exactly; the covering axis fills the canvas and the other centers a black-rendering `overhang`. Returns the resolved five values, `unusedCanvas`, `overhang`, `impliedAspect`, per-axis overlap `limits` (omitted for a `contentScale` request), each output's resulting `slice`, warnings, and the working copy's `revision`/`generation`. Never writes; same `422`s as the `PUT` below, plus a slice pushed entirely off the canvas. See [Grid arrangement](configuration.md#grid-arrangement). |
...
- It will still be fulfilled *after a restart* when `committed` is also `true`. A working copy (`PUT /config` with `committed: false`) is applied to the outputs exactly as a saved document is, so an appliance can be `synced` against a document that vanishes the moment it reboots. A working copy that changes only `projection.temporary`, the calibration aids that are never saved, still reports `committed: true`: a restart loses nothing but the aid.
...
| `PUT` | `/config/projection/arrangement` | Solve a rows × columns grid (per-axis overlap, or a content scale) and write each enabled output's `geometry.slice`; `committed: false` (default) previews, `true` persists. Returns the full document, like any other config write, with the resolved record as `projection.arrangement`. See [Grid arrangement](configuration.md#grid-arrangement). |
| `GET` | `/config/projection/arrangement` | The resolved record of the last applied arrangement, whether re-solving it still reproduces the current slices (`inEffect`), and (when computable) its per-axis overlap `limits`, always as if `allowUnusedCanvas` were `false`. |
...
```jsonc
{
  ...
  "settings": {
    "hideCursor": true,                             // implemented via `seat * hide_cursor 1000` + park offscreen
...
- **Layout is the client's job, with one exception.** Positions and projection geometry are always explicit and stay the canonical stored form; Suede does no general layout arithmetic (no "row" auto-layout of `position`). The reference UI offers left-to-right arrangement as a client-side convenience that simply computes `position` values, and any other client can do the same. The exception, added 2026-09-21: solving a regular overlapping grid for the shared projection canvas (rows, columns, independent X/Y overlap, or a content scale) is arithmetic every slider-driven client needs to agree on, so the daemon exposes it as `GET`/`PUT /config/projection/arrangement` and a dry-run `GET /projection/arrangement`; the reference UI's arrangement dialog calls the same endpoint instead of solving locally. The endpoint only *writes* explicit `geometry.slice` values, exactly as a hand-computed one would. See [Grid arrangement](configuration.md#grid-arrangement).
...
- **Browser resolution.** The kiosk presets search several names, because no single one is right everywhere: Debian and Arch ship `chromium`, Raspberry Pi OS and older Ubuntu `chromium-browser`, Google's own package `google-chrome-stable`. A snap is passed over as though absent — it updates itself on its own schedule and restarts the browser when it does, which on an appliance blanks the displays mid-show — and a machine carrying only a snap is reported as having no browser, naming the snap it declined. `launcher.program` overrides the search outright, including with a snap, in which case the profile is placed where a confined snap can write.
...
- The appliance boot chain (auto-login via agetty override, Sway launch from the login shell, disabling competing desktop environments and display blanking) is handled by the shipped `provision.sh` (see [Environment preparation](#environment-preparation-and-health-checks)), not by the daemon.
...
1. **Automatic output layout** — rejected for general positioning; reversed 2026-09-21 for grid arrangement specifically. Layout arithmetic still belongs in clients for arbitrary positions, and explicit geometry remains the canonical stored form. The exception: a slider-driven client arranging a regular overlapping grid needs one solver that is correct, fast, and identical for every client watching the same wall, which only the daemon can guarantee — so grid solving moved into the daemon as `PUT`/`GET /config/projection/arrangement` and a dry-run `GET /projection/arrangement`, and the reference UI's **Arrange automatically** dialog now calls it instead of computing locally. (See [Grid arrangement](configuration.md#grid-arrangement) and [Data model](#data-model).)
