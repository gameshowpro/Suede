...
| `GET` | `/projection/arrangement` | Read-only dry run of the grid-arrangement solver for the current working copy: query `rows`, `columns`, and either `overlapX`+`overlapY` or `contentScale`. Returns the resolved five values, `unusedCanvas`, `impliedAspect`, each output's resulting `source`, warnings, and the working copy's `revision`/`generation`. Never writes; same `422`s as the `PUT` below. See [Grid arrangement](configuration.md#grid-arrangement). |
...
| `GET` | `/config/projection/arrangement` | The resolved record of the last applied arrangement, and whether re-solving it still reproduces the current sources (`inEffect`). |
