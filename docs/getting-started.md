...
5. If the surface is not flat, or the projectors cannot be racked perfectly square, switch the mode to **Warp** and drag each output's corner and center-line handles until its picture is square on the physical surface. See [Editing geometry in the web UI](configuration.md#geometry-editor). To fine-tune the seams, turn on **Highlight overlaps** in the edge-blending panel and adjust until each orange and blue pair merges into one white line. It is never saved. See [Highlighting overlaps](configuration.md#highlight-overlaps).
